/**
 * Update : https://github.com/langchain-ai/langgraphjs/blob/main/libs/checkpoint-sqlite/src/index.ts
 * to use Bun's SQLite. Their build of better-sqlite3 is incompatible with Bun's
 */
...
export class BunSqliteSaver extends BaseCheckpointSaver {
    ...
    static fromConnString(connStringOrLocalPath: string): BunSqliteSaver {
        return new BunSqliteSaver(new Database(connStringOrLocalPath));
    }
...
CREATE TABLE IF NOT EXISTS writes (
  ...
  PRIMARY KEY (thread_id, checkpoint_ns, checkpoint_id, task_id, idx)
    ...
    async *list(
      ...
      FROM checkpoints\n`;
        ...
        const args = [
            ...
            ...Object.values(sanitizedFilter).map((value) =>
        ...
        if (rows) {
            for (const row of rows) {
                ...
                const pending_sends = await Promise.all(
                    (JSON.parse(row.pending_sends) as PendingSendColumn[]).map((
                        send,
    ...
    async put(
        ...
        const [type1, serializedCheckpoint] = this.serde.dumpsTyped(
            preparedCheckpoint,
        );
    ...
    async putWrites(
        ...
        if (!config.configurable?.checkpoint_id) {
            throw new Error(
                "Missing checkpoint_id field in config.configurable.",
            );
        }
